...
class LakeShore331:
    """
    Interface class for Lake Shore Model 331 Temperature Controller.

    The 331 is similar to the 335 but has only one heater output instead of two.
    This class provides methods to communicate with the Lake Shore 331 via PyVISA,
    allowing control of temperature setpoints, PID parameters, ramping, and reading
    of all instrument parameters.
    ...
    Attributes:
        instrument: PyVISA instrument object for communication
        inputs: List of available sensor inputs ['A', 'B']
        outputs: List of available control outputs [1] (331 has only one output)
    """
    ...
    def __init__(self, resource: str, read_termination: str = '\r\n',
                 ...
                 data_bits: int = 7, parity: int = 1, backend: str = '@py'):
        """
        Initialize connection to Lake Shore 331.
        ...
        # Available inputs and outputs (331 has only ONE output)
        self.inputs = ['A', 'B']
        self.outputs = [1]  # Only one output for 331

        # Test connection
        try:
            self.get_identification()
        except Exception as e:
            raise ConnectionError(f"Failed to connect to Lake Shore 331: {e}")
    ...
    def _validate_output(self, output: int):
        """Validate output channel is 1 (331 has only one output)."""
        if output not in self.outputs:
            raise ValueError(f"Output must be {self.outputs[0]} for Lake Shore 331")
        return output
    ...
    def set_T(self, output: int, temperature: float):
        """
        Set temperature setpoint for output.

        Args:
            output: Output channel (must be 1 for 331)
            temperature: Target temperature
        """
        output = self._validate_output(output)
        self._write(f"SETP {output},{temperature}")
    ...
    def get_setpoint(self, output: int) -> float:
        """
        Get temperature setpoint for output.
    ...
    def set_ramp(self, output: int, rate: float):
        """
        Set temperature ramp rate for output.
    ...
    def ramp_on(self, output: int):
        """
        Enable ramping for output (keeps current rate).
    ...
    def get_ramp_rate(self, output: int) -> float:
        """
        Get current ramp rate for output.
    ...
    def get_ramp_status(self, output: int) -> bool:
        """
        Get ramping enable status for output.
        ...
        response = self._query(f"RAMP? {output}")
        parts = response.split(',')
        return bool(int(parts[0]))

    # ==================== PID Control ====================

    def get_PID(self, output: int) -> Tuple[float, float, float]:
        """
        Get PID parameters for output.
    ...
    def autotune_PID(self, output: int, mode: int = 1):
        """
        Initiate PID autotuning for output.
    ...
    def get_zones(self, output: int) -> List[Tuple[float, float]]:
        """
        Get zone boundaries for output.
    ...
    def set_zones(self, output: int, zones: List[Tuple[float, float]]):
        """
        Set zone boundaries for output.
    ...
    def get_all_parameters(self) -> Dict[str, Any]:
        """
        Read all parameters stored in the Lake Shore 331.
        ...
        try:
            ...
            # Output parameters (only output 1 for 331)
            for output_ch in self.outputs:
                output_data = {}
                output_data['setpoint'] = self.get_setpoint(output_ch)
                output_data['heater_output'] = self.get_heater_output(output_ch)
