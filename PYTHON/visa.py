...
class LakeShore335:
    ...
    def get_all_parameters(self) -> Dict[str, Any]:
        ...
        The returned dictionary is organized into logical sections:
        - identification: Instrument identification and basic info
        - inputs: Configuration and readings for input channels A and B
        - outputs: Configuration and status for output channels 1 and 2  
        - control: PID parameters, setpoints, ramping, and zone settings
        - alarms: Alarm configurations and status
        - system: Display, interface, and other system settings
        
        Returns:
            Dictionary containing all instrument parameters organized by category.
            Each parameter includes both the raw value and a human-readable description
            where applicable.
            
        Example:
            >>> params = controller.get_all_parameters()
            >>> print(f"Input A temperature: {params['inputs']['A']['temperature_K']} K")
            >>> print(f"Output 1 setpoint: {params['outputs'][1]['setpoint']} K")
            >>> print(f"PID values: P={params['control']['pid'][1]['P']}")
            
        Note:
            This method performs many queries and may take several seconds to complete.
            Use specific getter methods if you only need certain parameters.
        """
        all_params = {
            'identification': {},
            'inputs': {},
...
if __name__ == "__main__":
    ...
    try:
        with LakeShore335('ASRL3::INSTR') as controller:
            ...
            # Get all parameters (comprehensive readout)
            # all_params = controller.get_all_parameters()
            # print("All parameters retrieved successfully")
